---
title: 故障排查
description: 排查 SynapS3 setup、健康检查、钱包、缓存、任务和 provider 常见问题。
---

# 故障排查

当用户可见流程失败时，从这里开始。先检查 health，再按 wallet、cache、task 或 provider 信号缩小范围。
...
如果 health 不是 `ok`，优先按返回的 error 文本继续排查。
...
这表示 SynapS3 因缺少必要配置进入 setup mode，常见原因是缺少 `filecoin.private_key`。
...
如果进程刚重启，启动恢复会释放 stale leases。若 worker 持续不健康，先记录当前任务状态并查看日志，再重启服务。

## 钱包 funding 或 deposit 失败
...
- 恢复 provider 或 worker 连接，让排队上传完成并触发缓存淘汰。
...
只有在底层依赖修复后再重试。常见依赖包括 RPC 连接、provider 可用性、钱包余额或缓存磁盘容量。
