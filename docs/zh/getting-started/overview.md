...
存储提供方变为不可用后修复受影响副本，是一项独立且即将支持的产品能力。参见[计划支持的副本修复](../concepts/filecoin-storage-flow.md#计划支持的副本修复)。
...
- 根据运行方式选择 [Docker 部署](./docker.md)或[源码构建](./source.md)。
- 完成安装后，使用[快速开始](./quick-start.md)检查节点并上传第一个对象。
