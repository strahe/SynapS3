...
Login, session, and refresh responses contain `username`, `csrf_token`, `expires_at`, and `refresh_after`. After `refresh_after`, the dashboard requests renewal on the next trusted pointer, click, keyboard, or wheel interaction. Dashboard polling and returning to a visible tab do not renew the session. Active sessions have no absolute lifetime cap; a session without interaction expires near the configured lifetime, within one renewal interval of up to five minutes.
