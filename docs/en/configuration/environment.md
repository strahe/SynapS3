...
The Docker entrypoint also reads these variables. They control container startup paths and do not map to TOML config fields.

| Environment variable | Purpose |
| --- | --- |
| `SYNAPS3_DATA_DIR` | Runtime data directory used for automatic `synaps3 init`; defaults to `/var/lib/synaps3`. |
| `SYNAPS3_CONFIG` | Config file used by `synaps3 serve`; defaults to `$SYNAPS3_DATA_DIR/config.toml`. |
...
Use the TOML config file for stable settings that should survive process restarts and be visible in `synaps3 admin settings get`. Use `SYNAPS3_DATA_DIR` and `SYNAPS3_CONFIG` only for Docker entrypoint path control.
