...
func NewObjectStateMachine() *Machine {
	...
	m.Register(
		...
		Transition{From: string(model.ObjectStateUploading), To: string(model.ObjectStateStored)},
		...
		Transition{From: string(model.ObjectStateStored), To: string(model.ObjectStateFailed)},
