...
func TransitionState(ctx context.Context, m *Machine, u StateUpdater, id int64, from, to model.ObjectState) error {
	...
	return u.UpdateState(ctx, id, from, to)
}
...
func TransitionToFailed(ctx context.Context, m *Machine, u StateUpdater, id int64, from model.ObjectState, lastError string) error {
	if err := m.Validate(string(from), string(model.ObjectStateFailed)); err != nil {
		return fmt.Errorf("state transition rejected: %w", err)
	}
	return u.UpdateStateToFailed(ctx, id, from, lastError)
}
