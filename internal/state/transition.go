...
func TransitionState(ctx context.Context, m *Machine, u StateUpdater, versionID string, from, to model.ObjectState) error {
	...
	return u.UpdateVersionState(ctx, versionID, from, to)
}
...
func TransitionToFailed(ctx context.Context, m *Machine, u StateUpdater, versionID string, from model.ObjectState, lastError string) error {
	if err := m.Validate(string(from), string(model.ObjectStateFailed)); err != nil {
		return fmt.Errorf("state transition rejected: %w", err)
	}
	return u.UpdateVersionStateToFailed(ctx, versionID, from, lastError)
}
