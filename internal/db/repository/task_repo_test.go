...
func TestTaskRepo_CreateOrReactivateCancelledOnlyRequeuesCancelledTask(t *testing.T) {
	db := testDB(t)
	repos := repository.NewRepositories(db)
	ctx := context.Background()
	...
	replacement := &model.Task{
		...
		IdempotencyKey: key,
		Payload:        map[string]interface{}{"reason": "policy-switch"},
		Status:         model.TaskStatusQueued,
		MaxRetries:     7,
		ScheduledAt:    time.Now(),
	}
	activated, err := repos.Tasks.CreateOrReactivateCancelled(ctx, replacement)
	if err != nil {
		t.Fatalf("CreateOrReactivateCancelled(cancelled): %v", err)
	}
	if !activated {
		t.Fatal("CreateOrReactivateCancelled(cancelled) activated = false, want true")
	}
	...
	if got.Status != model.TaskStatusQueued ||
		got.Stage == nil ||
		*got.Stage != repository.CacheEvictionStageAfterUpload ||
		got.RefID != replacement.RefID ||
	...
	activated, err = repos.Tasks.CreateOrReactivateCancelled(ctx, replacement)
	if err != nil {
		t.Fatalf("CreateOrReactivateCancelled(completed): %v", err)
	}
	if activated {
		t.Fatal("CreateOrReactivateCancelled(completed) activated = true, want false")
	}
	got, err = repos.Tasks.GetByID(ctx, original.ID)
...
func TestTaskRepo_CancelActiveEvictionTasksExceptPreservesMatchingStageAndTerminalHistory(t *testing.T) {
	...
	afterUploadStage := repository.CacheEvictionStageAfterUpload
	tasks := []*model.Task{
		{
			Type:           model.TaskTypeEvictCache,
			Stage:          &lruStage,
	...
	cancelled, err := repos.Tasks.CancelActiveEvictionTasksExcept(ctx, lruStage, "policy changed")
	if err != nil {
		t.Fatalf("CancelActiveEvictionTasksExcept: %v", err)
	}
	if cancelled != 2 {
