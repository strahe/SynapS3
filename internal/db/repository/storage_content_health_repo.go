...
func (r *BunStorageContentRepo) ListBucketStorageHealthAffectedVersions(ctx context.Context, input BucketStorageHealthAffectedVersionsInput) (BucketStorageHealthAffectedVersionPage, error) {
	if input.BucketID <= 0 {
...
func (r *BunStorageContentRepo) listBucketStorageHealthAffectedVersionRows(ctx context.Context, input BucketStorageHealthAffectedVersionsInput, limit int) ([]affectedVersionRow, error) {
	filters, args := bucketStorageHealthAffectedVersionObjectFilters(r.db, input)
	args = append([]any{input.StaleBefore, input.BucketID, input.BucketID}, args...)
	...
	query := fmt.Sprintf(`WITH %s
		SELECT
			...
			COALESCE(risk_content.checksum, '') AS checksum,
			object_version.content_type,
			object_version.content_id,
			COALESCE(risk_cache.in_cache, FALSE) AS in_cache,
			CASE WHEN risk_object.current_version_id = object_version.version_id THEN TRUE ELSE FALSE END AS is_current,
			object_version.created_at,
			object_version.updated_at
		FROM object_versions AS object_version
		JOIN objects AS risk_object ON risk_object.id = object_version.object_id
		LEFT JOIN storage_contents AS risk_content ON risk_content.id = object_version.content_id
		LEFT JOIN object_cache AS risk_cache ON risk_cache.content_id = object_version.content_id
...
%s
		  AND EXISTS (
				...
				WHERE storage_copy.content_id = object_version.content_id
...
func (r *BunStorageContentRepo) listBucketStorageHealthRiskDataSetsForVersions(ctx context.Context, input BucketStorageHealthAffectedVersionsInput, versionIDs []string) (map[string][]BucketStorageHealthRiskDataSet, error) {
	out := make(map[string][]BucketStorageHealthRiskDataSet, len(versionIDs))
	if len(versionIDs) == 0 {
		return out, nil
	}
	dataSetFilter := ""
	args := []any{input.StaleBefore, input.BucketID, input.BucketID, bun.List(versionIDs)}
	...
	query := fmt.Sprintf(`WITH %s
		...
		JOIN storage_copies AS storage_copy ON storage_copy.storage_data_set_id = abnormal_data_set.local_data_set_id
...
func (r *BunStorageContentRepo) listBucketStorageHealthReadableAlternativeCounts(ctx context.Context, input BucketStorageHealthAffectedVersionsInput, versionIDs []string) (map[string]int, error) {
	...
	query := fmt.Sprintf(`SELECT
		...
		FROM object_versions AS object_version
		JOIN storage_copies AS readable_copy ON readable_copy.content_id = object_version.content_id
		JOIN storage_contents AS readable_upload
		  ON readable_upload.id = readable_copy.content_id
...
func bucketStorageHealthAffectedVersionObjectFilters(db bun.IDB, input BucketStorageHealthAffectedVersionsInput) (string, []any) {
...
func bucketStorageHealthAffectedVersionDataSetFilter(input BucketStorageHealthAffectedVersionsInput) (string, []any) {
	if input.LocalDataSetID <= 0 {
		return "", nil
	}
	return `
				  AND abnormal_data_set.local_data_set_id = ?`, []any{input.LocalDataSetID}
}
