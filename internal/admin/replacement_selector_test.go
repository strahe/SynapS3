...
func providerIDStrings(t *testing.T, selector *StorageProviderSelector) []string {
	t.Helper()
	providers, err := selector.ListReplacementProviderObservations(context.Background(), nil)
	if err != nil {
		t.Fatalf("ListReplacementProviderObservations: %v", err)
	}
	out := make([]string, 0, len(providers))
	for _, id := range providers {
		out = append(out, id.Facts.ProviderID.String())
	}
	return out
}
...
func TestListReplacementProviderObservations_AlsoListsUnavailableProviders(t *testing.T) {
	...
	if !reflect.DeepEqual(got, []string{"101", "202", "303"}) {
		t.Fatalf("providers = %v, want all observed providers", got)
	}
}
...
func TestListReplacementProviderObservations_IsStableAcrossRegistryOrder(t *testing.T) {
...
func TestListReplacementProviderObservations_SortsNumerically(t *testing.T) {
	got := providerIDStrings(t, selectorWithRegistry(
...
func TestListReplacementProviderObservations_ReportsAnUnavailableRegistry(t *testing.T) {
	if _, err := (*StorageProviderSelector)(nil).ListReplacementProviderObservations(context.Background(), nil); err == nil {
	...
	if _, err := selector.ListReplacementProviderObservations(context.Background(), nil); err == nil {
...
func TestListReplacementProviderObservations_AsksForTheSameSetTheTopologyShows(t *testing.T) {
	registry := &stubProviderRegistry{items: []observability.ProviderObservation{observedProvider("2", true, true)}}
	if _, err := NewStorageProviderSelector(registry).ListReplacementProviderObservations(context.Background(), nil); err != nil {
		t.Fatalf("ListReplacementProviderObservations: %v", err)
	}
	if registry.opts.Status != "" {
		t.Fatalf("requested status = %q, want every observed provider", registry.opts.Status)
	}
...
func TestListReplacementProviderObservations_ReadsPastTheFirstPage(t *testing.T) {
	...
	if len(got) != replacementProviderPageSize+1 || got[len(got)-1] != strconv.Itoa(replacementProviderPageSize+1) {
		t.Fatalf("provider count = %d, want all providers across both pages", len(got))
	}
...
func TestListReplacementProviderObservations_StopsWhenPaginationDoesNotAdvance(t *testing.T) {
